...
size_t lev_edit_distance(size_t len1, const CharT1 *string1,
                         ...
                         int xcost)
{
  ...
  if (xcost) {
    for (i = 1; i < len1; i++) {
      size_t *p = row + 1;
      const CharT1 char1 = string1[i - 1];
      const CharT2 *char2p = string2;
      size_t D = i - 1;
      size_t x = i;
      while (p <= end) {
        if (char1 == *(char2p++))
